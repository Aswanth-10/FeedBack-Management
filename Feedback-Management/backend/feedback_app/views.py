                ...
                for question in questions:
                    ...
                    if answer:
                        if question.question_type == 'multiple_choice':
                            ...
                            row.append(answer.text_answer or 'N/A')
                    else:
                        row.append('N/A')
            ...
            if responses.exists():
                ...
                for feedback_response in responses:
                    row = [
                        ...
                        feedback_response.ip_address[:15] if feedback_response.ip_address else 'N/A'
                    ]
                    ...
                    for question in questions:
                        ...
                        if answer:
                            if question.question_type == 'multiple_choice':
                                cell_value = answer.selected_option or 'N/A'
                            elif question.question_type == 'rating':
                                cell_value = str(answer.rating_value) if answer.rating_value else 'N/A'
                            else:  # text, textarea
                                cell_value = (answer.text_answer[:30] + '...') if answer.text_answer and len(answer.text_answer) > 30 else (answer.text_answer or 'N/A')
                        else:
                            cell_value = 'N/A'
...
class FeedbackResponseViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def export_all_excel(self, request):
        ...
        try:
            user = request.user
            responses = FeedbackResponse.objects.filter(form__created_by=user).order_by('-submitted_at')
                ...
                for feedback_response in responses:
                    for answer in feedback_response.answers.all():
                        ...
                        if question.question_type == 'multiple_choice':
                            ...
                            answer_text = (answer.text_answer[:50] + '...') if answer.text_answer and len(answer.text_answer) > 50 else (answer.text_answer or 'N/A')
    ...
    def export_analytics_excel(self, request):
        ...
        try:
            user = request.user
            forms = FeedbackForm.objects.filter(created_by=user).order_by('-created_at')
