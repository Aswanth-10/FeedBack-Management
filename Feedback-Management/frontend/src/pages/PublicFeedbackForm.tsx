...
const PublicFeedbackForm: React.FC = () => {
  ...
  const handlePhoneChange = (questionId: number, value: string) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      ...
      const phoneErrors = form?.questions.filter(q => 
        q.is_required && 
        q.question_type === 'phone' && 
        answers[q.id] && 
        !validatePhoneNumber(answers[q.id])
      );
      
      if (phoneErrors && phoneErrors.length > 0) {
        alert('Please enter valid phone numbers for all phone fields.');
        return;
      }
